...
import { AuthState, CommentItemProps } from "types/user";
...
const CommentItem: React.FC<CommentItemProps> = ({
  ...
  const recoilLogin = useRecoilValue<AuthState>(authState);
  const token: AuthState = recoilLogin.user?.token?.accessToken;

  const { _id } = useParams();
  const replyIdparse = reply + "";

  console.log("aalll,", alldata);
